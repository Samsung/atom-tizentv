var launchEmulatorManager = (function () {
    ...
    var EMULATOR_MANAGER_MAC =
        'emulator-manager.app/Contents/MacOS/emulator-manager';
    var EMULATOR_MANAGER_EXECUTABLE =
        process.platform == 'win32'
            ? 'emulator-manager.exe'
            : process.platform == 'linux'
            ? 'emulator-manager'
            : EMULATOR_MANAGER_MAC;
    ...
    return {
        ...
        handleCommand: function () {
            logger.info(
                moduleName,
                '==============================Run TV Emulator Manager start!'
            );
            sdkInstallPath = atom.config.get(
            ...
            if (sdkInstallPath) {
                ...
                var pathNotExist =
                    'Please configure your SDK path in Package Settings';
                common.showMsgOnWindow(
                    common.ENUM_WINMSG_LEVEL.ERROR,
                    pathNotExist
                );
            ...
            logger.info(
                moduleName,
                'The configured path of SDK is: ' + sdkInstallPath
            );
            logger.info(
                moduleName,
                'The path of Emulator is: ' + emulatorInstallPath
            );
            ...
            if (fs.existsSync(exePath)) {
                ...
                logger.info(
                    moduleName,
                    'Executing Emulator Manager tool: ' + startEmulatorCommand
                );
                ...
                innerProcess.exec(
                    ...
                    function (err, stdout, stderr) {
                        if (err) {
                            var launchErrStatus =
                                'Failed to start Emulator Manager!';
                            common.showMsgOnWindow(
                                common.ENUM_WINMSG_LEVEL.ERROR,
                                launchErrStatus
                            );
                            ...
                            logger.info(
                                moduleName,
                                'The Emulator Manager is quited!'
                            );
                            logger.info(
                                moduleName,
                                '==============================Run TV Emulator Manager end!'
                            );
                        }
                    }
                );
                ...
                var nullPathMsg =
                    'Tizen TV Emulator Manager is not installed in your Tizen Studio!';
                common.showMsgOnWindow(
                    common.ENUM_WINMSG_LEVEL.ERROR,
                    nullPathMsg
                );
